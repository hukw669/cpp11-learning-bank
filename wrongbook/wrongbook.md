...
这三条主线是当前复习重点。按每条路线从左到右学习：先读知识入口，再复述错题结论，最后完成对应题目；答错后再回到错题正文定位原因。

- [重点一：虚函数、虚表与多态边界](#focus-polymorphism)
- [重点二：值类别、移动语义与完美转发](#focus-move)
- [重点三：模板基础到 SFINAE](#focus-templates)

<a id="focus-polymorphism"></a>
### 重点一：虚函数、虚表与多态边界

**推荐顺序：** [O8 继承与运行时多态](../sections/02_类模板STL.md?plain=1#L431) → [W039 虚函数与虚表](#w039) → [W037 `override` 与 `final`](#w037) → [W005 动态库与插件 ABI](#w005) → [题目 288–297（O8）](../questions/batch_10_271_300.md#288节点o81概念公有继承表达什么关系派生对象中有什么)。

复习时把以下链条连起来：虚函数建立动态分派，虚表和虚指针是主流实现手段而非标准强制；虚析构保证经基类接口正确销毁，对象切片会丢失派生部分，公共虚函数和类布局变化还可能破坏动态库 ABI。

**过关要求：** 能说明一次虚调用如何选择最终覆盖函数，能判断何时需要虚析构，并能解释按值保存派生对象为什么失去多态。

<a id="focus-move"></a>
### 重点二：值类别、移动语义与完美转发

**推荐顺序：** [R4 值类别与引用](../sections/02_类模板STL.md?plain=1#L653) → [W014 将亡值](#w014) → [R5 移动语义](../sections/02_类模板STL.md?plain=1#L687) → [W015 移动语义知识链](#w015) → [W036 复制构造与复制赋值](#w036) → [T7 转发引用与完美转发](../sections/02_类模板STL.md?plain=1#L1794) → [题目 314–323（R4–R5）](../questions/batch_11_301_330.md#314节点r41分类表达式-xstdmovex-和-t-分别属于哪种值类别) → [题目 367–370（T7）](../questions/batch_13_361_390.md#367节点t71判断哪些-t-是转发引用)。

复习时先判断表达式的值类别，再判断引用绑定和重载选择；`std::move` 只把表达式转为将亡值，真正转移资源取决于随后选中的操作，常见的是移动构造或移动赋值。进入泛型代码后，再用引用折叠解释 `T&&`，用 `std::forward<T>` 保留调用者的值类别。

**过关要求：** 能区分纯右值与将亡值，能解释具名 `T&&` 为什么是左值，并能说明 `std::move`、移动操作和 `std::forward` 各自负责哪一步。

<a id="focus-templates"></a>
### 重点三：模板基础到 SFINAE

模板内容以主题章节和题库作为主学习入口，按难度分三段：

1. [T1–T3：模板模型、函数模板、类模板](../sections/02_类模板STL.md?plain=1#L907)，完成[题目 342–354](../questions/batch_12_331_360.md#342节点t11概念模板和预处理宏的根本区别是什么)。
2. [T4–T6：特化、依赖名、可变参数模板](../sections/02_类模板STL.md?plain=1#L1342)，依次完成[题目 355–360](../questions/batch_12_331_360.md#355节点t41特化类模板全特化的语法和作用是什么)和[题目 361–366](../questions/batch_13_361_390.md#361节点t53语法依赖对象调用成员模板时为什么要写-template)。
3. [T7：完美转发](../sections/02_类模板STL.md?plain=1#L1794) → [T8：类型萃取与 SFINAE](../sections/02_类模板STL.md?plain=1#L1968)，完成[题目 367–374](../questions/batch_13_361_390.md#367节点t71判断哪些-t-是转发引用)。

关联错题可回看 [W013 `decltype`](#w013)、[W015 移动与完美转发](#w015)、[W016 `auto`](#w016) 和 [W017 类型别名](#w017)。当前错题本没有为 T1–T8 的每个节点虚构独立错题；后续实际答错或主动追问时，再按维护规则更新已有条目或新增历史编号。

**过关要求：** 能写出最小函数模板和类模板，解释推导、实例化与特化的区别，并能说明参数包展开、依赖名消歧、类型萃取和 SFINAE 分别解决什么问题。

## 全部错题目录（历史编号）

以下目录按历史编号排列；删除留下的编号空缺不回填，现有条目不因重点顺序而重排。
...
<a id="w041"></a>
## W041：`std::list` 与 `std::deque` 的底层结构有什么区别？

**问题：** `std::list` 的底层是不是“一个 map 再指向多个连续内存数组”的二级结构？`list`、`deque` 和 `vector` 到底怎样区分？

**核心答案：** “一个内部索引表指向多个连续内存块”描述的是 `std::deque` 的主流分段存储实现，不是 `std::list`。这里常被实现文档称为 `map` 的对象是内部指针表或索引表，**不是关联容器 `std::map`**。`std::list` 的主流实现是独立节点组成的双向链；`std::vector` 则保证所有元素位于一个连续序列中。ISO C++11 规定这些容器的接口、复杂度和失效规则，但除 `vector` 的元素连续性等明确要求外，不强制采用某一种内部字段布局。

### 1. 先认清三种典型结构

```text
std::list 的主流实现：双向节点链

[prev | A | next] ⇄ [prev | B | next] ⇄ [prev | C | next]


std::deque 的主流实现：内部索引表 + 多个元素块

内部索引表（有些实现称 map，但不是 std::map）
┌─────┬─────┬─────┐
│  *  │  *  │  *  │
└──┬──┴──┬──┴──┬──┘
   │     │     │
...
`deque` 的单个块内部通常连续，但相邻逻辑元素可能跨越两个块。因此整个 `deque` 不保证是一个连续数组，不能从 `&deque[0]` 开始用指针算术跨越全部元素。应使用下标或迭代器访问。

### 2. 标准保证与常见实现必须分开

C++11 对 `std::list<T>` 保证的重点包括：

- 它是顺序容器，提供双向迭代器。
- 不提供 `operator[]`，迭代器也不能执行 `it + n`。
- 已知位置的单元素插入和擦除具有常数复杂度。
- 插入不会使已有元素的迭代器和引用失效；擦除只使指向被擦除元素的位置失效。
- 满足前置条件的 `splice` 能转移节点，并保持指向被转移元素的迭代器和引用有效。

对 `std::deque<T>`，C++11 保证的重点包括：

- 支持随机访问迭代器和 `operator[]`，随机访问为常数复杂度。
- 在两端插入或删除单个元素为常数复杂度。
- 在中间插入、删除时，需要移动到操作点较近一端之间的元素，复杂度为线性。
- 不保证全部元素位于单块连续内存中。

标准没有要求 `list` 的节点“一定恰好两个指针”或“一定使用哨兵”，也没有要求 `deque` 一定具有名为 `map` 的字段、固定块大小或某种求块号公式。节点链、环形哨兵、指针表和分块大小都属于主流实现模型，不能据此编造可移植的 `sizeof` 或依赖内部地址布局。

`list` 节点和 `deque` 元素块通常通过容器的分配器取得，但这不等于标准要求每次操作都直接调用一次系统堆分配函数；实现和自定义分配器可以采用不同策略。

### 3. `list` 为什么能在已知位置 `O(1)` 插入和删除？

如果已经取得位置迭代器，插入节点只需让新节点接入相邻节点之间；擦除节点则让它的前驱和后继互相连接。被操作位置之外的元素不需要移动。
...
“常数时间插入”有一个经常被漏掉的条件：**位置已经找到**。如果只有“把元素插到第 1000 个位置”或“删除值等于 42 的元素”这样的要求，仍要先从链表中遍历定位，定位过程是 `O(n)`。因此整个任务不一定是 `O(1)`。

### 4. `deque` 的二级结构怎样支持随机访问和两端操作？

在典型分段实现中，`deque` 迭代器或容器会记录当前元素所在的块以及块内位置。随机访问时，实现可以根据逻辑下标计算目标块和块内偏移，所以标准接口能提供 `O(1)` 下标访问；这不要求所有元素物理连续。

向头部或尾部插入时，通常先使用当前端点块的空位；块满后再取得一个新块，并让内部索引表增加相应入口。必要时索引表本身也可能重新布置，但已有元素不必像 `vector` 扩容那样全部搬到同一块新内存中。

在 `deque` 中间插入或删除则不同：实现需要把操作点到较近一端之间的元素移动，复杂度为线性。因此“分块”主要服务于随机访问和两端增长，不会让任意位置插删都变成 `O(1)`。

### 5. 最小 C++11 示例：同时观察 `deque` 与 `list`
...
int main()
{
    ...
    int& middle = queue[1];
    queue.push_back(40); // 末端插入会使迭代器失效，但已有元素的引用保持有效
    assert(middle == 20);
    assert(queue[2] == 30); // 随机访问为 O(1)

    std::list<int> destination{1, 4};
    std::list<int> source{3, 2};

    std::list<int>::iterator moved = source.begin();
    destination.splice(destination.end(), source, moved);

    // moved 仍然有效，现在指向 destination 中的同一个元素 3。
    assert(*moved == 3);
    assert(source.size() == 1);

    destination.sort(); // 不能写 std::sort(destination.begin(), destination.end())
    assert(destination.front() == 1);
    assert(destination.back() == 4);
}
```

`std::sort` 要求随机访问迭代器，因此可以用于 `deque`，却不能用于只提供双向迭代器的 `list`；链表应调用成员 `list::sort()`。`list::sort()` 可以通过重新连接节点完成，不要求把所有元素放进连续数组。

### 6. 两种容器的迭代器和引用何时失效？

对 `list` 的正常插入、`push_front` 和 `push_back`：
...
- 只使指向被删除元素的迭代器、指针和引用失效。
- 其他元素的位置保持有效。
- 遍历删除时应接住 `erase` 返回的后继迭代器。

```cpp
for (std::list<int>::iterator it = destination.begin();
     it != destination.end(); )
{
    if (*it % 2 == 0)
...
被删除节点的生命周期已经结束，不能再次解引用旧位置。容器销毁时，指向其中所有元素的位置都会失效。

`deque` 的 C++11 失效规则更复杂：

- 在头部或尾部插入元素会使所有迭代器失效，但不会使指向已有元素的引用失效。
- 在中间插入元素会使所有迭代器和引用失效。
- 如果擦除范围包含最后一个元素，只有被擦除元素的位置和旧的尾后迭代器必然失效。
- 如果擦除范围包含第一个元素但不包含最后一个元素，只有指向被擦除元素的位置失效。
- 如果擦除既不包含第一个元素也不包含最后一个元素的中间范围，所有迭代器和引用都会失效。

这解释了为什么不能把 `deque` 的迭代器稳定性当作 `list`，也不能因为两端插入保留已有元素引用，就误以为旧迭代器仍然可用。涉及 `end()` 的代码尤其应在修改后重新取得尾后迭代器。

### 7. `list::splice` 为什么特殊？

`splice` 的价值是把已有节点从一个位置接到另一个位置，通常只重连链接，不复制或移动元素 `T`。被转移元素的迭代器和引用保持有效，并在操作后指向目标链表中的同一元素。
...
| 转移一个元素 | 常数 |
| 同一链表内转移一个范围 | 常数 |
| 不同链表间转移一个范围 | 与范围长度成线性关系 |

源链表和目标链表的分配器必须相等，否则行为未定义。整表转移要求源对象和目标对象不是同一个链表；区间转移时，插入位置不能位于被转移范围内部。单元素在同表内转移到自身位置或紧邻其后的等价位置时不发生变化。满足这些条件之后，才可以用“重连节点”理解 `splice`。

### 8. 为什么链表遍历可能比 `vector` 慢？

大 O 复杂度没有描述全部机器成本：

- `list` 节点通常分散在内存中，顺序遍历需要不断追踪链接。
- 每个节点除 `T` 外还需要链接、对齐和分配管理等额外开销，具体大小由实现决定。
- 分散地址通常难以充分利用 CPU 缓存和预取。
- 创建、销毁大量独立节点可能增加分配器开销。
- `vector` 的元素连续，即使中间插入需要移动元素，连续遍历和尾部追加仍经常更快。

所以“链表插入是 `O(1)`”不能推出“链表性能一定优于 `vector`”。应根据访问方式、元素大小、修改位置是否已知、地址稳定性要求和真实基准测试来选择。

### 9. 与其他顺序容器比较

| 容器 | 存储与访问特点 | 适合的典型需求 |
| --- | --- | --- |
| `vector` | 连续存储、随机访问，扩容或中间修改可能使位置失效 | 频繁遍历、按下标访问、尾部增长 |
| `deque` | 分段连续、支持随机访问和两端操作，不保证整体连续 | 频繁首尾插入删除，同时需要下标访问 |
| `list` | 节点式双向遍历，已知位置插删和节点转移稳定 | 已持有位置、要求元素地址稳定、需要 `splice` |
| `forward_list` | 单向节点链，只能向前遍历，常通过前驱位置修改 | 只需单向遍历并希望减少双向链接负担 |

`forward_list` 在 C++11 中提供 `before_begin`、`insert_after` 和 `erase_after`，没有 `size()`、`back()` 或 `push_back()` 成员。不要仅凭容器名称假设其接口与 `list` 对称。

### 10. 易错点

```text
内部索引表 + 多个连续块
≠ std::list；这是 std::deque 的典型实现模型
